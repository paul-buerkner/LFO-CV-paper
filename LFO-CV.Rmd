...
"$+$" operator following common programming language definitions.
...
\propto \frac{
p(\theta^{(s)}_{1:L}) \prod_{j \in 1:i} p(y_j \,|\, y_{1:(j-1)}, \theta^{(s)}) 
}{
p(\theta^{(s)}_{1:L}) \prod_{j \in 1:i^\star} p(y_j \,|\, y_{1:(j-1)}, \theta^{(s)}) 
}
...
both presented in @vehtari2019psis, any PSIS approximation will be very close to the
exact value as long as the Pareto $k$ diagnostic does not exceed the threshold
of $0.7$, which we used as the refit criterion in our approximate LFO-CV
approach. Second, there is uncertainty induced by the finite number of posterior
draws but this uncertainty should be small with just a few thousand draws
[@vehtari2017loo] and can easily be reduced further by sampling more draws. Third,
there is uncertainty caused by finite amounts of data. One could use an
analogous approach to what is done in approximate LOO-CV by computing the
standard error across the pointwise estimates for each observation
[@vehtari2017loo]. However, this requires the assumption of conditional
independence, which is not justified at least for $M$-step-ahead predictions
with $M > 1$. Investigating uncertainty measures for (approximate) LFO-CV 
...
\theta^{(s)}_{1:i^\star}$ are draws from the posterior distribution 
based on the first $1:i^\star$ observations. In backward LFO-CV, we start
using _all_ observations, that is, set $i^\star = N$. To obtain $w_i^{(s)}$, we
...
$i^\star = i^\star_1$ and restarting the process.
An illustration of this procedure is shown
