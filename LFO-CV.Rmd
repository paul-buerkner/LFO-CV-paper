...
of great practical relevance. For many time series models, inference could be
performed using sequential Monte Carlo (SMC) [e.g., @doucet2000; @andrieu2010]
implemented for example in the SMC specific probabilistic programming framework
Birch [@birch]. Most SMC algorithms are using importance sampling, and thus
LFO-CV could be obtained as by-product of running SMC and computation would
resemble the approach presented here. The proposal distribution at each step and
the applied "refit" approach, when the importance sampling weights degenerate,
are specific to each SMC algorithm. As we focus on generic probabilistic
programming frameworks, implementation details of LFO-CV for SMC algorithms are
...
they are very computationally intensive. Importantly, we do not make any
...
$\tau$ when $\tau$ is within the interval $[0.5,0.7]$ motivated in \@ref(psis)
