...
Perform M-step-ahead predictions (M-SAP)
$$
...
\begin{center}
We are moving \textbf{backwards} in time!
\end{center}
...
Let's call $J_i$ the index set of observations included in the approximating model but **not** in the target model

For observation $i$ and posterior sample $s$ we compute the importance ratio as
$$
r_i^{(s)} = \frac{1}{\prod_{j \in J_i} p(y_j \,|\, \,\theta^{(s)})}
$$
...
```{r 1sap, message=FALSE, fig.height=3.5, fig.width=5}
lfo_sims %>% 
  filter(M == 1) %>% 
  ...
  geom_label(
    aes(x = 5.2, y = 1.8, label = rel_nrefits), 
...
```{r 4sap, message=FALSE, warning=FALSE, fig.height=3.5, fig.width=5}
lfo_sims %>% 
  filter(M == 4) %>% 
  ggplot(aes(x = elpd_diff_lfo, y = ..density..)) +
  facet_grid(
  ...
  geom_vline(xintercept = 0, linetype = 2) +
  geom_label(
    aes(x = 22, y = 0.25, label = rel_nrefits), 
...
- PSIS-LFO-CV improves speed by an order of magnitude
...
## Approximate Block M-Step-Ahead Predictions
