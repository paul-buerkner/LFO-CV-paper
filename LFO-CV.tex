...
values are shown in Figure \ref{fig:block1sap}. PSIS-LFO-CV provides almost unbiased estimate of the corresponding exact LFO-CV for all investigated conditions, regardless
...
corresponding 1-SAP simulations. In particular, PSIS-LFO-CV has small bias compared to the  exact LFO-CV. However, the accuracy of
...
\({\rm ELPD}_{\rm exact} =\) -93.4 and
\({\rm ELPD}_{\rm approx} =\) -91.7,
...
\({\rm ELPD}_{\rm exact} =\) -538.7 and
\({\rm ELPD}_{\rm approx} =\) -539.6,
...
\({\rm ELPD}_{\rm exact} =\) -88.6 and
\({\rm ELPD}_{\rm approx} =\) -88.0,
which are highly similar. Plotting the Pareto \(k\) estimates reveals that the
...
\({\rm ELPD}_{\rm exact} =\) -484.3 and
\({\rm ELPD}_{\rm approx} =\) -488.8,
...
-2349.5 and \({\rm ELPD}_{\rm approx} =\)
-2346.4, which are highly similar. Plotting the
...
\ref{fig:pareto-k-lh}). On average, this means one refit every 18.6 observations, which implies a drastic speed increase as compared
