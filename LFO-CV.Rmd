...
{\rm ELPD} = \sum_{i=L+1}^{N - M + 1} 
  \int p_t(\tilde{y}_{i<M}) \log p(\tilde{y}_{i<M} \,|\, y_{<i})
  \, {\rm d} \, \tilde{y}_{i<M}.
\end{equation}

The distribution $p_t(\tilde{y}_{i<M})$ describes the true data generating
process for new data $\tilde{y}_{i<M}$. As the true data generating processes
are unknown, we approximate the ELPD using LFO-CV, which leads to
 
\begin{equation}
{\rm ELPD}_{\rm LFO} = \sum_{i=L+1}^{N - M + 1} \log p(y_{i<M} \,|\, y_{<i}).
\end{equation}
...
\begin{equation}
p(y_{i<M} \,| \, y_{<i}) = 
  \int p(y_{i<M} \,| \, y_{<i}, \theta) \, p(\theta\,|\,y_{<i}) \, {\rm d} \theta. 
\end{equation}
...
\mathbb{E}_f[h(\theta)] = \int h(\theta) f(\theta) \,d\, \theta 
 = \int \frac{[h(\theta) f(\theta) / g(\theta)] g(\theta)}
    {[f(\theta) / g(\theta)] g(\theta)} \,d\, \theta 
...
@vehtari2017psis] and has been demonstrated to have lower error and faster
convergence rate than other commonly used reqularization technigues
...
of the generalized Pareto distribution fitted to the largest weights provides
...
has finite variance, and based on the central limit theorem, the convergence of
the estimate is fast. As a result, approximate LOO-CV via PSIS is highly
accurate [@vehtari2017psis]. For $0.5 \leq k < 1$, a generalized central limit
...
\propto \frac{1}{\prod_{j \in J_i} p(y_j \,|\, \,\theta^{(s)})},
\end{equation}
...
blossom using an approximate Gaussian process model (cite) to
