...
median absolute deviation (MAD). The reason for focussing on the ELPD is that,
in contrast to RMSE and MAD, the ELPD evaluates the whole distribution rather
than just point estimates to provide a measure of out-of-sample predictive
performance, which we see as favorable from a fully Bayesian perspective 
[@vehtari2012]. The code we provide on GitHub
...
manner, that is, as increments per observation. In Appendix XXX, we
provide additional simulation results for the RMSE.
...
raw importance ratios $r_i(\theta^{(s)})$ as explained in @vehtari2017psis, and $\theta^{(s)}$ are draws from the
...
performance for reasons outlined in Section \ref{m-sap}. In Appendix XXX, we provide additional simulations for the RMSE. 
...
would not be the case if $\tau$ was allowed to be larger; @vehtari2017psis]. For
