...
equations computed for previous observations. Observations from an AR process are 
therefore not conditionally independent by definition, but the likelihood is
still factorizable because we can write down a separate contribution for each
...
the accuracy of the PSIS approximation is independent of the threshold
$\tau$ when $\tau$ is within the interval $[0.5,0.7]$ motivated in \@ref(psis-MSAP)
(this would not be the case if $\tau$ were allowed to be larger). 
The proportion of observations at which refitting the model was required
did not exceed $3\%$ under any of the conditions and only increased minimally when
...
that value. As expected, LOO-CV (the lighter histograms in Figure
\@ref(fig:1sap)) is a biased estimate of the 1-SAP performance for all
non-constant models, in particular for models with a trend in the time-series.
...
the future will therefore be have much greater uncertainty and, as a result, the
PSIS-LFO-CV approximation becomes more uncertain as well. This is not a
limitation of the method, but rather an artifact of the limited predictive
capabilities of these particular models.
