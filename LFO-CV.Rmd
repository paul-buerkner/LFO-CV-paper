...
value $i = N - M + 1$, we have $J_i = \{i, \ldots, N\}$. This approach to
...
up to index $N^\star_1 = i^\star_1 - 1$. An illustration of this 
...
while retaining all of the more distant future values. This approach is
...
(see Section \@ref(simulations)), the observations in the left-out block have to
...
More formally, let $\overline{J}_i$ be the index set of observations that are
...
if $\max(i + B, N^\star + 1) \leq \min(N^\star + B, N)$ and $\overline{J}_i =
\emptyset$ otherwise. As above, $N^\star$ refers to the largest observation
included in the model fitting ($N^\star = i^\star - 1$, where $i^*$ is the index
of the latest refit). The raw importance ratios $r_i^{(s)}$ for each posterior
draw $s$ are then computed as
