...
```{r vis-msap, fig.width=8, fig.height=3, fig.cap="Visualisation of PSIS approximated one-step-ahead predicitions. The `x` represents the currently predicted observation. In the shown example, the model was last refit at the 7th observation."}
df <- data.frame(
  obs = rep(1:9, 3),
  i = factor(rep(5:7, each = 9)),
  Status = c(
    rep("included", 4), rep("PSIS adjusted", 2), rep("left out", 3),
    rep("included", 5), rep("PSIS adjusted", 1), rep("left out", 3),
    rep("included", 6), rep("left out", 3)
  )
) %>%
  mutate(
    Status = factor(Status, levels = c("left out", "included", "PSIS adjusted"))
  )

ggplot(df, aes(obs, i, fill = Status)) +
  geom_tile(height = 0.9, width = 1, col = "black") +
  annotate(
    'text', x = c(5, 6, 7), y = c(1, 2, 3), 
    label = "x",
    parse = TRUE, size = 10
  ) +
...
${\rm ELPD}_{\rm exact} =$ `r round(elpd_exact_lfo_block1sap_lh, 2)` and 
${\rm ELPD}_{\rm approx} =$ `r round(elpd_approx_lfo_block1sap_lh, 2)`, 
which are highly similar. Plotting the Pareto $k$ estimates reveals that the
model had to be refit `r nrefits` times, out of a total of $N - L =$ 
...
${\rm ELPD}_{\rm exact} =$ `r round(elpd_exact_lfo_block4sap_lh, 2)` and 
${\rm ELPD}_{\rm approx} =$ `r round(elpd_approx_lfo_block4sap_lh, 2)`, 
...
`r round(sum_exact_lfo_1sap_cb, 2)` and ${\rm ELPD}_{\rm approx} =$ 
`r round(sum_approx_lfo_1sap_cb, 2)`, which are highly similar. Plotting the 
...
\ref{fig:pareto-k-lh}). On average, this means one refit every `r round((N - L)
/ nrefits, 2)` observations, which implies a drastic speed increase as compared
to exact LFO-CV. Performing LFO-CV of 4-SAP leaving out all future values, we
compute ${\rm ELPD}_{\rm exact} =$ `r round(sum_exact_lfo_4sap_cb, 2)` and ${\rm
ELPD}_{\rm approx} =$ `r round(sum_approx_lfo_4sap_cb, 2)`, which are again
...
0.6$. As the number of required model refits, and thus the computiational time,
