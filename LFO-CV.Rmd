...
LOO-CV is fine for time series model assessment if we are interested in the
performance of interpolation within the time series or the local conditional
distribution. However, if we are interested in the performance of extrapolation 
to new future time points, leaving out only one observation at a time allows
information from the future to influence predictions of the past (i.e., times 
$t+1, t+2, \ldots$ should not be used to predict time $t$). To apply the idea
of cross-validation to the $M$-SAP case, we need some form of leave-*future*-out
...
some form of prediction of future time points. Like exact LOO-CV, exact LFO-CV
requires refitting the model many times to different subsets of the data, which
is computationally costly for most nontrivial examples, in particular for
Bayesian inference where refitting the model means estimating a new posterior
...
number times and will make LFO-CV tractable for many more realistic applications
...
Pareto distribution fitted to the largest raw importance ratios. This procedure is called
Pareto smooth importance sampling [PSIS; @vehtari2017loo; @vehtari2017psis] and
has been demonstrated to have lower error and faster convergence rate than other
commonly used reqularization technigues
...
of the generalized Pareto distribution fitted to the largest weights
provides information about the number of existing moments of the
weight distribution and the actual importance sampling estimate.
When $k<0.5$, the weight distribution has finite variance, and
based on the central limit theorem, the convergence of the estimate is
fast. As a result, approximate LOO-CV via PSIS is highly accurate
[@vehtari2017psis]. For $0.5 \leq k < 1$, a generalized central limit
theorem holds, but the convergence rate drops quickly when $k$
increases [@vehtari2017psis].  In practice, PSIS has been shown to be
relatively robust for $k < 0.7$ [@vehtari2017loo;
@vehtari2017psis]. As such, the default threshold is set to $0.7$ when
performing PSIS LOO-CV [@vehtari2017loo].  
...
in PSIS-LFO-CV. If there is a large error when leaving out $i$th observation,
then there is likely to be a large error when leaving out observations $i-1,
i-2, \ldots$ until a refit is performed. That is, highly influential
observations with high $k$ are likely to have stronger effects for total
estimate in LFO-CV than LOO-CV. We will come back to the issue of setting
...
may exceed $\tau$ rather quickly (i.e., after only few observations) and so a
lot of refits may be required even when carrying out the PSIS approximation to
LFO-CV. In this case, another option is to exclude only the block of future
values that directly follow the observations to be predicted while retaining all
of the more distant values. This will usually result in lower Pareto $k$
estimates and thus less refitting, but crucially alters the underlying
prediction task, to which we will refer to as block-$M$-SAP. As the  
of application of block-$M$-SAP is less common, we do not 
discuss this approach in the main part of this paper, but refer the
...
estimate of 1-SAP performance for all
...
$\tau =$ `r k_thres` for the Pareto $k$ estimates at which define that refitting 
...
highly similar. Not only is the overall ELPD estimated accurately but also
...
`r fmt(sum_lh_elpd_4sap_approx, 1)`, which are again similar, but not as close
as in the 1-SAP case. In general for increasing $M$, the approximation tends to
become less accurate in absolute ELPD units, as the ELPD increment of each
observation will be based on more and more observations. Moreover, we see some
...
\@ref(fig:lh-pw-elpd)). Since, for constant threshold $\tau$, the importance
weights are the same independent of $M$, Pareto $k$ estimates are also the same
in $4$-SAP as in $1$-SAP.
...
lh_pw_elpd <- tibble(
  ...
  k = rev(attributes(lh_elpd_1sap_approx)$ks),
  M = "M = 1"
) %>% bind_rows(
  tibble(
    elpd_exact = na.omit(lh_elpd_4sap_exact),
    elpd_approx = na.omit(lh_elpd_4sap_approx),
    k = rev(attributes(lh_elpd_4sap_approx)$ks),
    M = "M = 4"
  )
)
...
ks <- attributes(lh_elpd_1sap_approx)$ks
ids <- N:(L + 1)
plot_ks(ks, ids)
```
...
blossom using an approximate Gaussian process model (TODO: what to cite?) to
...
${\rm ELPD}_{\rm approx} =$ `r fmt(sum_cb_elpd_1sap_approx, 1)`, which are similar
given the high the number of predicted observations (`r N - L`), but not as
close as we want them to be. More specifically, PSIS-LFO-CV slightly
overestimates the predictive performance for $\tau =$ `r k_thres`, which is in
line with our simulation results (see Section \@ref(simulations)). As shown
in the left panel of Figure \@ref(fig:cb-pw-elpd), visible differences
between pointwise exact and approximate ELPD contributions only arise for very 
few observations. Since
these differences all go in the same direction, they induce a slight bias in
the overall ELPD estimate. Still, PSIS-LFO-CV clearly performs better than
PSIS-LOO-CV for which we obtain ${\rm ELPD}_{\rm exact} =$ 
`r fmt(loo_cb$estimates[1, 1], 1)` and thus an even strong overestimation of the
predictive performance. Plotting the Pareto $k$ estimates reveals that the model
had to be refit `r nrefits` times, out of a total of $N - L =$ `r N - L`
predicted observations (see Figure \ref{fig:cb-pareto-k}). On average, this
means one refit every `r fmt((N - L) / nrefits, 1)` observations, which implies
a drastic speed increase as compared to exact LFO-CV.

Performing LFO-CV of 4-SAP, we compute ${\rm ELPD}_{\rm exact} =$ 
`r fmt(sum_cb_elpd_4sap_exact, 1)` and ${\rm ELPD}_{\rm approx} =$ 
`r fmt(sum_cb_elpd_4sap_approx, 1)`, which are again similar and do not show 
a stronger bias than the corresponding 1-SAP results
(although, in expectation, the bias of PSIS-LFO-CV will increase for increasing
$M$). This is because, for this example, differences between the pointwise exact
and approximate ELPD contributions go in both direction thus partly canceling
each other out (see the right panel of Figure \@ref(fig:cb-pw-elpd)). Since, 
for constant threshold $\tau$, the importance weights are the same independent
of $M$, Pareto $k$ estimates are also the same in $4$-SAP as in $1$-SAP.
...
cb_pw_elpd <- tibble(
  ...
  k = rev(attributes(cb_elpd_1sap_approx)$ks),
  M = "M = 1"
) %>% bind_rows(
  tibble(
    elpd_exact = na.omit(cb_elpd_4sap_exact),
    elpd_approx = na.omit(cb_elpd_4sap_approx),
    k = rev(attributes(cb_elpd_4sap_approx)$ks),
    M = "M = 4"
  )
)
...
ks <- rev(attributes(cb_elpd_1sap_approx)$ks)
ids <- (L + 1):N
plot_ks(ks, ids)
```
...
for reasons discussed in Appendix B, we currently do not recommend using
block-$M$-SAP in practice.

For a set of common time-series models, we established via simulations that
PSIS-LFO-CV is almost unbiased approximation of exact LFO-CV if we choose the
...
PSIS_LFO_CV = function(model, data, M, L, tau) {
  ...
  for (i in (N - M + 1):(L + 1)) {
    ...
    if (k > tau) {
      ...
      i_star = i - 1
      model_refit = update(model_refit, data = data[-(i:N), ])
...
## Appendix B: Block $M$-step-ahead predictions {-#approximate_blockMSAP}
...
lfo_sims %>% 
  ...
  rename(`$\\tau$` = "k_thres") %>%
  kable(
    caption = "Mean percentages of required refits for block-$M$-SAP.",
    booktabs = TRUE,
    escape = FALSE
  ) %>%
  footnote(
...
\ref{approximate_blockMSAP}). This implies a disproportionally high variability
...
ks <- attributes(lh_elpd_block1sap_approx)$ks
ids <- N:(L + 1)
plot_ks(ks, ids)
```
...
such as autoregressive models for details). Together with the fact that
block-$M$-SAP is only theoretically reasonable for stationary time series, as the
future will always be informative for non-stationary ones, this leaves PSIS
...
it might thus be more consistent and safe to just use PSIS-LFO-CV of $M$-SAP not
trying to approximate block-$M$-SAP at all.
