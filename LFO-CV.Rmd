...
process for new data $\tilde{y}_{i<M}$. As these true data generating processes
...
In case of factorizable models the response values are conditionally 
...
In this case, $p(y_{i<M} \,|\, y_{<i}, \theta_{<i}^{(s)})$ reduces to 
\begin{equation}
p(y_{i<M} \,|\, \theta_{<i}^{(s)}) = 
  \prod_{n = i}^{i + M -1} p(y_j \,|\, \theta_{<i}^{(s)}),
\end{equation}
due to the assumption of conditional independence between $y_{i<M}$ and $y_{<i}$
given $\theta_{<i}^{(s)}$. Cross-validation for non-factorizable models, which 
do not make this assumption, is discussed in @buerkner:non-factorizable.
...
We thank Daniel Simpson and Shira Mitchell for helpful comments and discussions
