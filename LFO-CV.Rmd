...
($M$-SAP). Fortunately, once we have fit a Bayesian model and can sample from the
...
decisions *before* we can collect the future observations required for assessing
...
to shorten the notation. The quantities $p(y_{i<M} \,|\, y_{<i})$ can be
computed with the help of the posterior distribution $p(\theta \,|\, y_{<i})$ of
the parameters $\theta$ conditional on only the first $i-1$ observations of the
time-series:
...
are discussed in @buerkner:non-factorizable.
...
and then stabilize them using PSIS as described in @vehtari2017psis. The index
...
[@vehtari2017psis]. In practice, PSIS-LOO-CV turned out to be robust for 
$k < 0.7$ [@vehtari2017loo]. That is, for PSIS-LFO-CV introduced in the
...
## Block $M$-step-ahead predictions
...
(see Section X), the observations of the left-out block have to be modeled as
missing values in order to retain the integreity of the time-series'
predictions after the block. A related example from spatial statistics, in which
the modeling of missing values is required for valid inference, can be found in
@buerkner:non-factorizable.
...
also illustrated graphically in Figure \ref{simmodels}.
...
block_lfo_res <- lfo_sims %>% filter(!is.na(B), M == 1)
lfo_res_4sap <- lfo_sims %>% filter(is.na(B), M == 4)
block_lfo_res_4sap <- lfo_sims %>% filter(!is.na(B), M == 4)
```
...
fit_lh <- brm(
  ...
  control = list(adapt_delta = 0.95), chains = 2,
  file = "models/fit_lh"
)
```
...
${\rm elpd}_{\rm exact} =$ `r round(sum_exact_lfo_1sap_lh, 2)` and 
${\rm elpd}_{\rm approx} =$ `r round(sum_approx_lfo_1sap_lh, 2)`, 
...
this means one refit every `r round((N - L) / nrefits, 2)` observations, which
...
${\rm elpd}_{\rm exact} =$ `r round(sum_exact_lfo_4sap_lh, 2)` and 
${\rm elpd}_{\rm approx} =$ `r round(sum_approx_lfo_4sap_lh, 2)`, 
which are again similar. In general, for increasing $M$, the approximation tends
to become less accurate in absolute elpd units, as the elpd increment of each
...
```{r pareto-k-lh, fig.height=2.5, fig.cap="Pareto $k$ estimates for PSIS-LFO-CV of 1 and 4 step-ahead predictions leaving out all future values. The dotted red line indicates the threshold at which the refitting was necessary."}
...
sum_exact_lfo_block1sap_lh <- summarize_elpds(exact_lfo_block1sap_lh)[1]
sum_approx_lfo_block1sap_lh <- summarize_elpds(approx_lfo_block1sap_lh)[1]
```
...
sum_exact_lfo_block4sap_lh <- summarize_elpds(exact_lfo_block4sap_lh)[1]
sum_approx_lfo_block4sap_lh <- summarize_elpds(approx_lfo_block4sap_lh)[1]
```
...
${\rm elpd}_{\rm exact} =$ `r round(sum_exact_lfo_block1sap_lh, 2)` and 
${\rm elpd}_{\rm approx} =$ `r round(sum_approx_lfo_block1sap_lh, 2)`, 
which are highly similar. Plotting the Pareto $k$ estimates reveals that the
model had to be refit `r nrefits` times, out of a total of $N - L =$ 
...
${\rm elpd}_{\rm exact} =$ `r round(sum_exact_lfo_block4sap_lh, 2)` and 
${\rm elpd}_{\rm approx} =$ `r round(sum_approx_lfo_block4sap_lh, 2)`, 
...
```{r pareto-k-lh-block, fig.height=2.5, fig.cap="Pareto $k$ estimates for PSIS-LFO-CV of 1 and 4 step-ahead predictions leaving out a block of 10 future values. The dotted red line indicates the threshold at which the refitting was necessary.", fig.lp="test"}
...
## Annual date of the cherry blossom in Japan
...
plot(marginal_effects(fit_cb), points = TRUE, plot = FALSE)[[1]] +
